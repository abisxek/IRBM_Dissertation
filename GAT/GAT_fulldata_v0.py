...
RNA_PATH      = os.path.join(DATA_DIR, "outputs/train_rna_tuned_elbow.h5ad")
...
# BINN gene-importance CSVs (produced by binn_gene_extraction.py, consumed
# by binn_gene_selection_tuning.py). Used here a second time — not to pick
# genes, but to weight the already-selected genes before SVD so that SVD's
# variance-maximizing components are biased toward the axes BINN flagged
# as protein-predictive, instead of treating every selected gene equally.
IMPORTANCE_DIR = os.path.join(DATA_DIR, "outputs")
SHAP_CSV       = os.path.join(IMPORTANCE_DIR, "gene_shap_importance.csv")
GRAD_CSV       = os.path.join(IMPORTANCE_DIR, "gene_dense_importance.csv")
IMPORTANCE_W_MIN = 0.2   # weight floor — keeps low-importance genes from
                         # being zeroed out entirely (they still selected by
                         # BINN, just less so; don't discard their variance)
IMPORTANCE_W_MAX = 1.0   # weight ceiling for the most important genes

# SVD config (replaces GMT / pathway config).
# Lowered from 256 -> 128: 256 components on the ~2k-gene BINN-selected
# panel is a much more aggressive compression ratio (256/2000 ≈ 12.8% of
# the feature space) than 256 was on the original 18k-gene matrix
# (256/18000 ≈ 1.4%). Fewer components restores a similar relative
# compression ratio and leaves less room for near-duplicate components
# drawn from a smaller, more collinear gene panel.
...
# ── Normalise ──────────────────────────────────
log("Normalising RNA using precomputed (full-transcriptome) size factors...")
assert "size_factor" in rna.obs.columns, \
    "size_factor missing — re-run gene selection tuning with the size-factor fix"

size_factors = rna.obs["size_factor"].values.astype(np.float32)
size_factors = np.where(size_factors == 0, 1.0, size_factors)  # avoid div-by-zero

target_sum = 1e4
if sp.issparse(rna.X):
    inv_sf = sp.diags(target_sum / size_factors)
    rna.X = (inv_sf @ rna.X).tocsr()
else:
    rna.X = rna.X * (target_sum / size_factors[:, None])
...
# ── Weight genes by BINN importance before SVD ─
# TruncatedSVD is unsupervised — left alone it maximizes variance with no
# regard for which genes actually predict protein. Scaling each gene's
# (log-normalised) expression by its BINN importance score before fitting
# SVD biases the resulting components toward the axes BINN already told us
# matter, instead of whatever has the most incidental variance in the
# curated panel.
log("Loading BINN importance weights for SVD input scaling...")
gene_weights = load_gene_importance_weights(
    rna_gene_list, SHAP_CSV, GRAD_CSV,
    w_min=IMPORTANCE_W_MIN, w_max=IMPORTANCE_W_MAX,
)
np.save(os.path.join(DATA_DIR, "gene_importance_weights.npy"), gene_weights)
log(f"  Weight range: [{gene_weights.min():.3f}, {gene_weights.max():.3f}]  "
    f"(saved to gene_importance_weights.npy for reuse at inference time)")

X_rna_weighted = X_rna_sparse @ sp.diags(gene_weights)

svd = TruncatedSVD(
    n_components = N_SVD_COMPONENTS,
    random_state = RANDOM_SEED,
)
score_matrix_final = svd.fit_transform(X_rna_weighted).astype(np.float32)
...
log(f"Compression ratio (genes : components) = "
    f"{len(rna_gene_list)}:{N_SVD_COMPONENTS} "
    f"({len(rna_gene_list) / N_SVD_COMPONENTS:.1f}x)")
...
config = {
    ...
    "importance_weighting": {
        "enabled": True,
        "w_min": IMPORTANCE_W_MIN,
        "w_max": IMPORTANCE_W_MAX,
        "shap_csv": SHAP_CSV,
        "grad_csv": GRAD_CSV,
    },
...
# Apply the SAME gene-importance weights used at training time before
# transforming — svd.transform() expects input scaled the same way the
# fitted SVD saw it, otherwise the projection is silently wrong.
log("Applying saved BINN importance weights to validation matrix...")
X_valid_weighted = X_valid_full @ sp.diags(gene_weights)

# Apply the SAME (already-fit) SVD model
log("Applying trained SVD transform to validation data...")
t_start = time.time()
score_matrix_v = svd.transform(X_valid_weighted).astype(np.float32)
