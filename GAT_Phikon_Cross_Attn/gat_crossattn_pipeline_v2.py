...
This version matches the ORIGINAL architecture (hardcoded gat1/gat2 layers)
used to train the proj1024 checkpoint that scored 0.67 on the STP Open
Challenge validation set - no configurable depth/fusion-type options, so
no state_dict key remapping is needed to load that checkpoint.

The only change from the original version: run_predict now uses mini-batch
NeighborLoader-based inference instead of a single full-batch forward pass.
Full-batch inference worked fine at validation scale (~8,786 bins) but
OOMs on the larger test set (~84,386 bins) - same underlying issue as
full-batch GAT training, fixed the same way (only small sampled subgraphs
are moved to GPU per step, not the whole graph at once).
...
2) PREDICT on new RNA-only data (e.g. test_rna.h5ad), producing a
   submission-ready CSV (barcode, pxl_row_in_fullres, pxl_col_in_fullres,
   then 44 protein columns), with automatic inverse transform to raw
   protein intensity units:

    python gat_crossattn_pipeline.py predict \
        --model_path outputs_gat_crossattn_proj1024/model_weights.pt \
        --valid_rna_input data/test_rna.h5ad \
        --train_rna_reference data/train_rna_processed.h5ad \
        --valid_image_embeddings outputs_histology_test/embeddings.npy \
        --valid_image_index outputs_histology_test/embedding_index.csv \
        --output_path outputs_gat_crossattn_proj1024/test_predictions_raw.csv \
        --num_neighbors 15 10 \
...
NeighborLoader support - needed for both train and predict now):
...
def run_predict(args):
    ...
    edge_index = build_spatial_knn_graph(coords, args.k_neighbors)  # kept on CPU for NeighborLoader

    log(f"Validation graph built: {edge_index.shape[1]} directed edges")

    # ------------------------------------------------------------------
    # Mini-batch inference via NeighborLoader. A single full-batch forward
    # pass (whole graph on GPU at once) works fine at validation scale
    # (~8,786 bins) but OOMs on larger test sets (~84,386 bins) - same
    # underlying memory issue as full-batch training, fixed the same way:
    # only small sampled subgraphs are moved to GPU per step.
    # ------------------------------------------------------------------
    ...
    infer_loader = NeighborLoader(
        ...
        shuffle=False,  # preserve order so predictions map back to bins correctly
    )

    n_bins = X_valid.shape[0]
    all_preds = np.zeros((n_bins, len(protein_names)), dtype=np.float32)
    filled = np.zeros(n_bins, dtype=bool)

    start = time.time()
    with torch.no_grad():
        for batch in infer_loader:
            # n_id maps each node in this sampled subgraph back to its
            # original global index in infer_graph - needed to place seed
            # node predictions into the correct rows of all_preds.
            seed_global_idx = batch.n_id[:batch.batch_size].cpu().numpy()
            batch = batch.to(device)
            pred = model(batch.x, batch.edge_index, batch.img_x)
            seed_pred = pred[:batch.batch_size].cpu().numpy()
            all_preds[seed_global_idx] = seed_pred
            filled[seed_global_idx] = True

    if not filled.all():
        n_missing = (~filled).sum()
        raise RuntimeError(f"{n_missing} bins were never covered as seed nodes during "
                            f"inference - check NeighborLoader configuration")
...
def build_parser():
    ...
    p_pred.add_argument("--num_neighbors", type=int, nargs="+", default=[15, 10],
                         help="Fan-out per GAT layer for mini-batch inference sampling - "
                              "should match the model's 2-layer architecture "
                              "(default: 15 10)")
    p_pred.add_argument("--inference_batch_size", type=int, default=1024,
                         help="Seed node batch size for mini-batch inference (default: "
                              "1024 - can be larger than training batch size since no "
                              "gradients/optimizer state need to be held in memory)")
