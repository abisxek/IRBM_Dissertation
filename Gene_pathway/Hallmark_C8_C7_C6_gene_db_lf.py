...
from torch_geometric.nn import GATConv          # ← GATConv not GATv2Conv
...
GMT_C7        = os.path.join(DATA_DIR, "c7_pathways.gmt")
GMT_C6        = os.path.join(DATA_DIR, "c6_pathways.gmt")
PRO_COFACTOR  = 150.0
CHUNK_SIZE    = 5000
RANDOM_SEED   = 42
N_BINS_SUBSET = None
...
MIN_GENES    = 10
combined_raw = {**hallmark, **c8, **c7, **c6}
log(f"Total gene sets before filter: "
    f"{len(combined_raw)}")
...
# No torch.compile — causes issues with DataLoader
n_params = sum(
...
def pearson_loss(pred, target):
    ...
    den      = (pred_m**2).sum(dim=0).sqrt() * \
               (target_m**2).sum(dim=0).sqrt()
    r        = num / (den + 1e-8)
    return (1 - r).mean()

def combined_loss(pred, target):
    return (0.5 * mse_criterion(pred, target) +
            0.5 * pearson_loss(pred, target))
...
OUTPUT_CSV = os.path.join(DATA_DIR, "submission_lf.csv")
